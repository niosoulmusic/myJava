...
public class Theatre {
    ...
    public Theatre(String theatreName, int numRows, int seatsPerRow) {
        ...
        for (char row='A';row <=lastRow;row++){
            for (int seatNum=1;seatNum<=seatsPerRow-1;seatNum++){
                double price= 12.00;
    ...
    public boolean reserveSeat(String seatNumber){
        ...
        if (foundSeat>=0) {
            ...
            System.out.println("There is no seats with number "+ seatNumber);
            return false;
        }
    }
    ...
    public class Seat implements Comparable<Seat>{
        ...
        public boolean cancel(){
            if (this.reserved){
                this.reserved=false;
                System.out.println("Cencelling reservation for seat number "+seatNumber);
                return true;
            }else{
